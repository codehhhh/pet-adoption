...
public class PetPostServiceImpl extends ServiceImpl<PetPostMapper, PetPost> implements PetPostService {
     ...
     */
    @Override
    public Result<String> top(Integer id) {
        // 使用 getBaseMapper().selectById 来获取实体，而不是 getById 方法
        PetPost petPost = getBaseMapper().selectById(id);
        if (petPost == null) {
            // 使用 ApiResult.error() 方法
            return ApiResult.error("帖子不存在");
        }
        if (!petPost.getIsAudit()) {
            return ApiResult.error("只能置顶已审核的帖子");
        }

        PetPost updateEntity = new PetPost();
        updateEntity.setId(id);
        updateEntity.setIsTop(true);
        updateById(updateEntity);
        return ApiResult.success("置顶成功");
    }
     ...
     */
    @Override
    public Result<String> cancelTop(Integer id) {
        PetPost petPost = getBaseMapper().selectById(id);
        if (petPost == null) {
            return ApiResult.error("帖子不存在");
        }
        ...
        PetPost updateEntity = new PetPost();
        updateEntity.setId(id);
        updateEntity.setIsTop(false);
        updateById(updateEntity);
        return ApiResult.success("取消置顶成功");
    }

    /**
     * 获取置顶的宠物经验帖子列表
     *
     * @return Result<List<PetPostListItemVO>>
     */
    @Override
    public Result<List<PetPostListItemVO>> getTopPosts() {
        // 构建查询条件，查询已审核且置顶的帖子
        LambdaQueryWrapper<PetPost> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(PetPost::getIsAudit, true) // 只查询已审核的
                .eq(PetPost::getIsTop, true)   // 查询置顶的
                .orderByDesc(PetPost::getCreateTime); // 按创建时间倒序

        List<PetPost> topPosts = list(queryWrapper);

        if (topPosts.isEmpty()) {
            // 使用 ApiResult.success() 返回空列表
            return ApiResult.success(new ArrayList<>());
        }

        // 获取帖子ID列表
        List<Integer> postIds = topPosts.stream()
                .map(PetPost::getId)
                .collect(Collectors.toList());

        // 获取完整的帖子信息
        List<PetPostListItemVO> topPostList = baseMapper.queryListItemByIds(postIds);
        return ApiResult.success(topPostList);
    }
